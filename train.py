...
grey_test = data_test[:, :, :, :1]
color_test = data_test[:, :, :, 1:]
...
for e in range(1, epochs):
    ...
    dis_results = 0
    grey_noise = utils.add_noise(grey)
    grey_test_noise = utils.add_noise(grey_test)
    while batch_counter < batch_total:
        color_batch = color[(batch_counter - 1) * batch_size:batch_counter * batch_size]
        grey_batch = grey[(batch_counter - 1) * batch_size:batch_counter * batch_size]
        grey_batch_noise = grey_noise[(batch_counter - 1) * batch_size:batch_counter * batch_size]
      
        batch_counter += 1
      
        toggle = not toggle
        if toggle:
            dis_input = np.concatenate((model_gen.predict(grey_batch_noise), grey_batch), axis=3)
            dis_output = np.zeros((batch_size, 1))
        else:
            dis_input = np.concatenate((color_batch, grey_batch), axis=3)
            dis_output = np.random.uniform(low=0.8, high=1.1, size=batch_size)
      
        dis_results = model_dis.train_on_batch(dis_input, dis_output)
      
        gen_input = grey_batch_noise
        dis_output = np.random.uniform(low=0.8, high=1.1, size=batch_size)
        gen_output = color_batch
    ...
    if (data_limit == -1):
        ...
        if e % 10 == 0:
            ev = model_gan.evaluate(grey_test_noise, [np.ones((grey_test_noise.shape[0], 1)), color_test])
    ...
    # evaluate on valid data        
        ...
        grey_valid = data_valid[:, :, :, :1]
        grey_valid_noise = utils.add_noise(grey_valid)
        color_valid = data_valid[:, :, :, 1:]
        
        if e % 10 == 0:
            ev = model_gan.evaluate(grey_valid_noise, [np.ones((grey_valid_noise.shape[0], 1)), color_valid])
            ev = np.round(np.array(ev), 4)
            summary = utils.create_summary_epoch(ev)
            writer.add_summary(summary, e)
