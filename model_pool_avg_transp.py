...
def create_model_gen(input_shape, output_channels):
    ...
    conv2 = create_conv(128, (4, 4), pool1, 'conv2', strides = 1, activation='leakyrelu')
    pool2 = AveragePooling2D((2,2))(conv2)
    
    conv3 = create_conv(256, (4, 4), pool2, 'conv3', strides = 1, activation='leakyrelu')
    pool3 = AveragePooling2D((2,2))(conv3)
    
    conv4 = create_conv(512, (4, 4), pool3, 'conv4', strides = 1, activation='leakyrelu')
    pool4 = AveragePooling2D((2,2))(conv4)
    
    conv5 = create_conv(512, (4, 4), pool4, 'conv5', strides = 1, activation='leakyrelu')
    pool5 = AveragePooling2D((2,2))(conv5)
    
    conv6 = create_conv_transpose(512, (4, 4), pool5, 'conv6', strides = 2, activation='leakyrelu')
    merge6 = concatenate([conv6, pool4], axis=3)
    
    conv7 = create_conv_transpose(512, (4, 4), merge6, 'conv7', strides = 2, activation='leakyrelu')
    merge7 = concatenate([conv7, pool3], axis=3)
    
    conv8 = create_conv_transpose(256, (4, 4), merge7, 'conv8', strides = 2, activation='leakyrelu')
    merge8 = concatenate([conv8, pool2], axis=3)
    
    conv9 = create_conv_transpose(128, (4, 4), merge8, 'conv9', strides = 2, activation='leakyrelu')
